...
for (const item of coursesAndDurationArray) {
    ...
    info.innerText = `${item.title} ------ ${item.monthDuration}`;
    document.body.appendChild(div.appendChild(info));
}
...
for (const item of coursesArray) {
    ...
    h3.innerText = `month duration - ${item.monthDuration}\n hour duration - ${item.hourDuration}`;
    let ul = document.createElement('ul');
