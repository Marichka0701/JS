...
buildStructure = (obj, innerContainer) => {
    for (const key in obj) {
        if (typeof (obj[key]) === 'object') {
            buildStructure(obj[key], innerContainer);
        } else {
...
for (const user of usersList) {
    ...
    buildStructure(user, innerContainer);
    ...
    for (let i = 0; i < element.childNodes.length; i++) {
        ...
        if (node.nodeType === Node.ELEMENT_NODE) {
            if (node.tagName === 'DIV' && node.classList.contains('title')) {
                obj.headings.push(node.textContent.trim());
            }
            traverseDOM(node);
        } else if (node.nodeType === Node.TEXT_NODE) {
            let text = node.textContent.trim();
            if (text.length > 0) {
                obj.paragraphs.push(text);
            }
        }
    }
}
